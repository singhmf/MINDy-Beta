function [Out,Kern] = DeconvHRF(X,ConvRes,WeinerNoise,varargin)
...
for i=1:size(X,1)
    [~,yy]=deconvblind(15+zscore(X(i,:)),ConvVec(1:25)+.3,20);
    cVec=[yy-yy(end) zeros(1,size(X,2)-length(yy))];
    Kern(i,:)=cVec; %#ok<NASGU>
    if i==1
