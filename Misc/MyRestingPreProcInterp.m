function [Y,BadFrames,Kern] = MyRestingPreProcInterp(XDat,FiltAmp,ConvLevel,DownSamp,TR,varargin)
...
%    gg
    ...
    X(:,isnan(mm))=[];
%    X(:,unique(tFilt))=[];
if numel(varargin)==2&&strcmpi(varargin{2},'y')
    [X,Kern{i}]=DeconvHRF(X,TR,ConvLevel,varargin{2});
else
